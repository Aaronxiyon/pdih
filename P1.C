...
//Funcion 5
void textcolor(int color){// modifica el color de primer plano con que se mostrar�n los caracteres
    union REGS inregs, outregs;
    inregs.h.ah = 0x09;
    inregs.h.bl = color;
    inregs.h.bh = 0x00;
...
//Funcion 6
void textbackground(int color){ //: modifica el color de fondo con que se mostrar�n los caracteres
    union REGS inregs, outregs;
    inregs.h.ah = 0x09;
    inregs.h.bl = color << 4;
    inregs.h.bh = 0x00;
	...
	while(terminado == 0)
	{
		...
		else if(tmp == '8')
      {
      	int letra, fondo, color;
			printf("\nEsta funcion escribe un caracter con los par�metros de color elegidos");
